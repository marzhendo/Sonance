...
- [ ] 1. Migration SQL untuk tabel `vc_sessions`
  - [ ] 1.1 Buat Alembic migration `backend/alembic/versions/0003_create_vc_sessions.py`
...
### Wave 1: ORM Model
- [ ] 2. ORM Model `VCSession`
  - [ ] 2.1 Buat ORM model `VCSession` di `backend/app/models/vc_session_model.py`
...
- [ ] 3. Pydantic Schemas untuk Protokol WebSocket Voice Changer
  - [ ] 3.1 Buat schemas di `backend/app/schemas/vc_session_schema.py`
...
- [ ] 4. Interface Pipeline dan Stub `RVCRealtimePipeline`
  - [ ] 4.1 Tambahkan interface `VoiceConversionPipeline(ABC)` di `backend/ml/base.py`
    - Abstract methods: `load_model(checkpoint_path)`, `unload_model()`, `is_loaded() -> bool`, `convert_chunk(pcm_data, settings) -> bytes`.
  - [ ] 4.2 Buat stub `RVCRealtimePipeline` di `backend/ml/rvc/__init__.py`
...
- [ ] 5. Integrasi Symmetrical GPU Lock pada `tts_worker.py`
  - [ ] 5.1 Perbarui `backend/workers/tts_worker.py` (`execute_tts_job`)
...
- [ ] 6. Implementasi `VCSessionManager`
  - [ ] 6.1 Buat service di `backend/app/services/vc_session_manager.py`
...
- [ ] 7. WebSocket Endpoint `/ws/voice-changer`
  - [ ] 7.1 Buat router `backend/app/routers/voice_changer_router.py` dan daftarkan di `backend/app/main.py`
  ...
  - [ ] 7.2 Integration tests di `backend/tests/integration/test_voice_changer_ws.py`
